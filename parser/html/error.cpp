...
namespace HTML::Tokenizer {
	const ParserError ParserError::ABRUBT_CLOSING_OF_EMPTY_COMMENT = ParserError(
	...
	const ParserError ParserError::ABRUBT_DOCTYPE_PUBLIC_IDENTIFIER
		= ParserError("abrupt-doctype-public-identifier",
	...
	const ParserError ParserError::ABRUBT_DOCTYPE_SYSTEM_IDENTIFIER = ParserError(
	...
	const ParserError ParserError::CDATA_IN_HTML_CONTENT = ParserError(
	...
	const ParserError ParserError::CONTROL_CHARACTER_IN_INPUT_STREAM
		= ParserError("control-character-in-input-stream",
	...
	const ParserError ParserError::CONTROL_CHARACTER_REFERENCE = ParserError(
	...
	const ParserError ParserError::END_TAG_WITH_ATTRIBUTES
		= ParserError("end-tag-with-attributes",
	...
	const ParserError ParserError::DUPLICATE_ATTRIBUTES
		= ParserError("duplicate-attribute",
	...
	const ParserError ParserError::END_TAG_WITH_TRAILING_SOLIDUS = ParserError(
	...
	const ParserError ParserError::EOF_BEFORE_TAG_NAME = ParserError(
		"eof-before-tag-name",
	...
	const ParserError ParserError::EOF_IN_CDATA = ParserError(
		"eof-in-cdata",
	...
	const ParserError ParserError::EOF_IN_COMMENT
		= ParserError("eof-in-comment",
	...
	const ParserError ParserError::EOF_IN_DOCTYPE = ParserError(
		"eof-in-doctype",
	...
	const ParserError ParserError::EOF_IN_TAG
		= ParserError("eof-in-tag",
	...
	const ParserError ParserError::INCORRECTLY_CLOSED_COMMENT = ParserError(
	...
	const ParserError ParserError::INCORRECTLY_OPENED_COMMENT
		= ParserError("incorrectly-opened-comment",
	...
	const ParserError ParserError::MISSING_ATTRIBUTE_VALUE
		= ParserError("missing-attribute-value",
	...
	const ParserError ParserError::MISSING_DOCTYPE_NAME
		= ParserError("missing-doctype-name",
	...
	const ParserError ParserError::MISSING_DOCTYPE_PUBLIC_IDENTIFIER
		= ParserError("missing-doctype-public-identifier",
	...
	const ParserError ParserError::MISSING_DOCTYPE_SYSTEM_IDENTIFIER
		= ParserError("missing-doctype-system-identifier",
					  "This error occurs if the parser encounters a U+003E (>) code point where start of the DOCTYPE "
					  "system identifier is expected (e.g., <!DOCTYPE html SYSTEM >). In such a case, if the DOCTYPE "
					  "is correctly placed as a document preamble, the parser sets the Document to quirks mode.");
	const ParserError ParserError::MISSING_END_TAG_NAME
		= ParserError("missing-end-tag-name",
	...
	const ParserError ParserError::MISSING_WHITESPACE_BETWEEN_DOCTYPE_PUBLIC_AND_SYSTEM_IDENTIFIERS = ParserError(
		...
		"ASCII whitespace. In this case the parser behaves as if ASCII whitespace is present.");
	const ParserError ParserError::NESTED_COMMENT
		= ParserError("nested-comment",
	...
	const ParserError ParserError::NONCHARACTER_CHARACTER_REFERENCE
		= ParserError("noncharacter-character-reference",
	...
	const ParserError ParserError::NONCHARACTER_IN_INPUT_STREAM = ParserError(
	...
	const ParserError ParserError::NULL_CHARACTER_REFERENCE = ParserError(
	...
	const ParserError ParserError::SURROGATE_CHARACTER_REFERENCE
		= ParserError("surrogate-character-reference",
					  "This error occurs if the parser encounters a numeric character reference that references a "
					  "surrogate. The parser resolves such character references to a U+FFFD REPLACEMENT CHARACTER.");
	const ParserError ParserError::SURROGATE_IN_INPUT_STREAM = ParserError(
	...
	const ParserError ParserError::UNEXPECTED_NULL_CHARACTER
		= ParserError("unexpected-null-character",
	...
	const ParserError ParserError::UNEXPECTED_QUESTION_MARK_INSTEAD_OF_TAG_NAME
		= ParserError("unexpected-question-mark-instead-of-tag-name",
					  ...
					  "point (if present) or to the end of the input stream is treated as a comment.");
	const ParserError ParserError::UNEXPECTED_SOLIDUS_IN_TAG
		= ParserError("unexpected-solidus-in-tag",
	...
	const ParserError ParserError::UNKNOWN_NAMED_CHARACTER_REFERENCE
		= ParserError("unknown-named-character-reference",
